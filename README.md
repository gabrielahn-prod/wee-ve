...
- ✅ 카드 디자인은 Figma("Weeve" 파일, node `50:93` "최종" 섹션의 1컷~8컷 프레임)에서 좌표를
  그대로 옮겨왔습니다 (`CARD_LAYOUTS`, `index.html` 내 `drawCard()` 근처). 장소를 1~8개 중
  몇 개를 입력하든 입력한 개수에 맞는 컷 디자인이 자동으로 쓰입니다 — 사람이 컷 수를 따로
  고르지 않아도 됩니다. 7컷/8컷은 균등한 그리드가 아니라 왼쪽 4칸(세로 480)+오른쪽
  3~4칸(세로 640/480)의 비정형 레이아웃이라 프레임별 실측 좌표를 그대로 하드코딩했습니다.
